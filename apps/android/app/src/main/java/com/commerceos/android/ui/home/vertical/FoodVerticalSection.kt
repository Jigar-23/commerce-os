...
@Composable
fun FoodVerticalSection(
    categories: List<VerticalCategory> = emptyList(),
    onOpenCatalog: (CatalogQuery) -> Unit
) {
    Column(verticalArrangement = Arrangement.spacedBy(Spacing.md)) {
        if (categories.isNotEmpty()) {
            Text("Cuisines & Dining Categories", style = CommerceTypography.Title, fontWeight = FontWeight.Bold, color = CommerceColors.TextPrimary)
            Row(
                ...
                for (cat in categories) {
                    Card(
                        ...
                        modifier = Modifier
                            .width(110.dp)
                            .clickable { onOpenCatalog(CatalogQuery(categoryId = cat.id, vertical = "food")) }
                    ) {
                        Column {
        ...
        Card(
            ...
            modifier = Modifier.fillMaxWidth().clickable { onOpenCatalog(CatalogQuery(vertical = "food")) }
        ) {
            Column(modifier = Modifier.padding(Spacing.lg)) {
                Text("Dining & Kitchens Catalog", style = CommerceTypography.Title, fontWeight = FontWeight.Bold, color = CommerceColors.TextPrimary)
                Spacer(modifier = Modifier.height(Spacing.sm))
                Text("Explore Food Catalog →", style = CommerceTypography.Meta, fontWeight = FontWeight.Bold, color = CommerceColors.Primary)
            }
        }
    }
}
