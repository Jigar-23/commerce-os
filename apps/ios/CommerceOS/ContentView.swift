...
struct IOSCheckoutView: View {
    ...
    @State private var addressLine: String = UserDefaults.standard.string(forKey: "saved_address_line") ?? ""
    @State private var city: String = UserDefaults.standard.string(forKey: "saved_city") ?? ""
    @State private var postalCode: String = UserDefaults.standard.string(forKey: "saved_postal_code") ?? ""
    @State private var latitude: Double = CLLocationManager().location?.coordinate.latitude ?? 0.0
    @State private var longitude: Double = CLLocationManager().location?.coordinate.longitude ?? 0.0
                ...
                Button(action: {
                    Task {
                        let addr = DeliveryAddressPayload(
                            addressLine: addressLine.isEmpty ? "Current GPS Location" : addressLine,
                            city: city.isEmpty ? "Local Area" : city,
                            postalCode: postalCode.isEmpty ? "000000" : postalCode,
                            latitude: latitude,
                            longitude: longitude
                        )
                        do {
                ...
                if addressLine.isEmpty {
                    CLGeocoder().reverseGeocodeLocation(loc) { placemarks, _ in
                        if let pm = placemarks?.first {
                            if self.addressLine.isEmpty {
                                self.addressLine = [pm.name, pm.subLocality].compactMap { $0 }.joined(separator: ", ")
                            }
