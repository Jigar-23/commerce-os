...
export default function OrdersPage() {
  ...
  const [rebroadcastOrderId, setRebroadcastOrderId] = useState<string | null>(null);
                    ...
                    <div className="shrink-0 flex items-center space-x-2.5">
                      ...
                      {(order.status === 'SELLER_ACCEPTED' || order.status === 'READY_FOR_PICKUP' || order.status === 'PACKED') && !order.riderId && order.orderStatus !== 'RIDER_ASSIGNED' && order.orderStatus !== 'OUT_FOR_DELIVERY' && order.orderStatus !== 'DELIVERED' && (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setRebroadcastOrderId(order.id);
                          }}
                          disabled={actionLoadingId === order.id}
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-border-default max-w-md w-full p-6 space-y-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-content-secondary font-mono">{rebroadcastOrderId}</p>
              </div>
            </div>
            ...
            <div className="flex justify-end space-x-3 pt-2">
              <button
                onClick={() => setRebroadcastOrderId(null)}
              ...
              <button
                onClick={async () => {
                  const targetId = rebroadcastOrderId;
                  setRebroadcastOrderId(null);
                  if (targetId) {
                    const mockEvent = { stopPropagation: () => {} } as any;
                    await handleAcceptOrder(mockEvent, targetId);
                  }
                }}
