...
async function runE2ETestSuite() {
  ...
  try {
    ...
    // E2E #5 & #6: Real Rider GPS Telemetry Stream with Delivery Association
    console.log('\n[E2E #5 & #6] Real GPS Telemetry Stream');
    await repos.telemetryRepo.recordTelemetry({
      riderId: 'rdr_01',
      deliveryId: testDeliveryId,
      sequenceNumber: 1,
      latitude: 28.2010,
    ...
    const latestTelem = await repos.telemetryRepo.getLatestTelemetryForDelivery(testDeliveryId);
    assert.ok(latestTelem, 'Telemetry must be retrievable by delivery ID');
    assert.strictEqual(latestTelem.deliveryId, testDeliveryId);
    assert.strictEqual(latestTelem.sequenceNumber, 1);
    assert.strictEqual(latestTelem.speedKmh, 24);
    recordPass('Telemetry delivery association & sequence number verified');
    ...
    // Route Persistence & Map Matching Verification
    console.log('\n[Route Engine] Route Geometry Persistence & Snapping');
    const waypoints = [
      { lat: 28.1989, lng: 76.6186 },
      { lat: 28.2010, lng: 76.6192 },
      { lat: 28.2100, lng: 76.6200 }
    ];
    await repos.deliveryRepo.updateRouteTransactionally(testDeliveryId, waypoints, 8, 2.5, 1);
    const sessionWithRoute = await repos.deliveryRepo.getDeliveryById(testDeliveryId);
    assert.strictEqual(sessionWithRoute.waypoints.length, 3);
    assert.strictEqual(sessionWithRoute.routeVersion, 1);
    recordPass('Route geometry and version persisted on delivery session');

    const enrichedTracking = buildEnrichedTrackingDTO(sessionWithRoute, latestTelem, null, sessionWithRoute.waypoints);
    assert.ok(enrichedTracking.routeProgressPct >= 0, 'Route progress computed');
    assert.strictEqual(enrichedTracking.telemetrySource, 'LIVE_TELEMETRY');
    assert.strictEqual(enrichedTracking.isStale, false);
    recordPass('Map-matched tracking DTO generated with authoritative route');
