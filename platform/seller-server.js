...
<body>
  ...
  <div class="app-body">
    ...
    <aside class="sidebar">
      <div>
        ...
        <button type="button" class="nav-btn" id="btn-riders" onclick="setTab('riders')">
          <div style="display: flex; align-items: center;">
            <span class="nav-icon">🛵</span>
            <span>Fleet & Riders</span>
          </div>
          <span id="badge-riders" class="nav-badge">0</span>
        </button>
    ...
    <main class="main-content">
      ...
      <!-- FLEET & RIDERS -->
      <section id="pane-riders" class="tab-pane">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
          <div>
            <h2 style="font-size: 18px; font-weight: 900;">Fleet & Delivery Partners</h2>
            <p style="font-size: 12px; color: #64748B;">Enroll new delivery partners and manage active rider roster.</p>
          </div>
          <button class="btn btn-emerald" onclick="openRiderModal()">+ Enroll New Rider</button>
        </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>Bike / Vehicle Reg</th>
                <th>Aadhaar KYC</th>
                <th>Status</th>
                <th style="text-align: right;">Action</th>
              </tr>
            </thead>
            <tbody id="riders-table">
              <tr><td colspan="6" style="text-align: center; color: #64748B; padding: 32px;">Loading fleet roster...</td></tr>
            </tbody>
          </table>
        </div>
      </section>
  ...
  <!-- ENROLL RIDER MODAL -->
  <div id="rider-modal" style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(3, 7, 18, 0.8); backdrop-filter: blur(4px); z-index: 999; display: none; align-items: center; justify-content: center;">
    <div class="modal-card">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; border-bottom: 1px solid #1E293B; padding-bottom: 12px;">
        <h3 style="font-size: 16px; font-weight: 900;">🛵 Enroll Delivery Partner</h3>
        <button class="btn btn-slate" onclick="closeRiderModal()">✕</button>
      </div>

      <form id="rider-form" onsubmit="handleSaveRider(event)">
        <div class="form-group">
          <label>Rider Full Name *</label>
          <input type="text" id="rf-name" class="input-box" required placeholder="e.g. Vikram Singh">
        </div>

        <div class="form-group">
          <label>10-Digit Mobile Number (Login ID) *</label>
          <input type="tel" id="rf-phone" class="input-box" required maxlength="10" placeholder="9991416180">
          <span style="font-size: 10px; color: #64748B; margin-top: 2px;">Rider uses this number to log into CommerceOS Rider App.</span>
        </div>

        <div class="grid-2">
          <div class="form-group">
            <label>Bike / Vehicle Reg. *</label>
            <input type="text" id="rf-vehicle" class="input-box" required placeholder="HR-26-AB-6180" style="text-transform: uppercase;">
          </div>
          <div class="form-group">
            <label>Vehicle Type</label>
            <select id="rf-type" class="input-box">
        ...
        <div class="form-group">
          <label>12-Digit Aadhaar Card Number (KYC)</label>
          <input type="text" id="rf-aadhaar" class="input-box" maxlength="14" placeholder="5678-1234-9012">
        </div>

        <div style="display: flex; justify-content: flex-end; gap: 8px; margin-top: 16px;">
          <button type="button" class="btn btn-slate" onclick="closeRiderModal()">Cancel</button>
          <button type="submit" class="btn btn-emerald">Enroll & Activate Partner</button>
        </div>
      </form>
    </div>
  <!-- PRESCRIPTION PREVIEW MODAL -->
  <div id="rx-modal" style="display: none; position: fixed; inset: 0; background: rgba(0, 0, 0, 0.85); backdrop-filter: blur(6px); z-index: 100; align-items: center; justify-content: center; padding: 20px;">
    <div style="background: #0F172A; border: 1px solid #334155; border-radius: 20px; max-width: 680px; width: 100%; max-height: 90vh; display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.7);">
      <div style="display: flex; justify-content: space-between; align-items: center; padding: 16px 20px; border-bottom: 1px solid #1E293B;">
        <div style="display: flex; align-items: center; gap: 10px;">
          <span style="background: rgba(239, 68, 68, 0.2); color: #F87171; border: 1px solid rgba(239, 68, 68, 0.4); padding: 3px 8px; border-radius: 6px; font-size: 11px; font-weight: 900;">Rx</span>
          <h3 style="font-size: 16px; font-weight: 900; color: #FFFFFF;" id="rx-modal-title">Doctor's Prescription</h3>
        </div>
        <button class="btn btn-slate" onclick="closeRxModal()">✕</button>
      </div>
      <div style="padding: 20px; overflow-y: auto; text-align: center; flex: 1;">
        <img id="rx-modal-img" src="" alt="Prescription" style="max-width: 100%; max-height: 55vh; border-radius: 12px; border: 1px solid #334155; object-fit: contain; background: #1E293B;" />
        <div id="rx-modal-meta" style="margin-top: 14px; text-align: left; background: #1E293B; border-radius: 12px; padding: 12px 16px; font-size: 12px; color: #94A3B8;"></div>
      </div>
      <div style="display: flex; justify-content: flex-end; gap: 10px; padding: 16px 20px; border-top: 1px solid #1E293B;">
        <button class="btn btn-slate" onclick="closeRxModal()">Close</button>
        <button id="rx-modal-reject-btn" class="btn btn-slate" style="color: #FB7185; border-color: rgba(251, 113, 133, 0.3);" onclick="handleRejectRxFromModal()">✕ Reject</button>
        <button id="rx-modal-approve-btn" class="btn btn-emerald" onclick="handleApproveRxFromModal()">✓ Verify &amp; Approve</button>
      </div>
    </div>
  </div>
...
const server = http.createServer(async (req, res) => {
  ...
  // POST /api/v1/orders/:id/verify-prescription
  const rxVerifyMatch = pathname.match(/^\/api\/v1\/orders\/([^/]+)\/verify-prescription$/);
  if (rxVerifyMatch && (req.method === 'POST' || req.method === 'PATCH')) {
    const orderId = decodeURIComponent(rxVerifyMatch[1]);
    const body = await parseJsonBody(req);
    const db = getDbData();
    const order = (db.orders || []).find(o => o.id === orderId || o.orderId === orderId);
    if (order) {
      const decision = body.decision || 'APPROVED';
      const licenseNo = body.licenseNo || 'PHARM-LIC-2026-9912';
      order.pharmacistVerification = {
        status: decision,
        pharmacistId: 'pharm_rewari_01',
        licenseNo: licenseNo,
        verifiedAt: new Date().toISOString(),
        notes: body.notes || 'Pharmacist verified clinical validity of attached prescription.'
      };
      if (decision === 'APPROVED') {
        order.status = 'SELLER_ACCEPTED';
        order.orderStatus = 'SELLER_ACCEPTED';
        order.sellerApprovalStatus = 'ACCEPTED';
        order.sellerApprovedAt = new Date().toISOString();
      } else {
        order.status = 'CANCELLED';
        order.orderStatus = 'CANCELLED';
        order.cancellation = { reason: 'PRESCRIPTION_REJECTED', notes: body.notes || 'Invalid or expired prescription.' };
      }
      saveDbData(db);
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify(order));
  ...
  if (pathname === '/api/v1/seller/riders' && req.method === 'GET') {
    const cloudRiders = await fetchFromGateway('/api/v1/seller/riders');
  ...
  if (pathname === '/api/v1/seller/riders' && req.method === 'POST') {
    const body = await parseJsonBody(req);
    const cloudRes = await postToGateway('/api/v1/seller/riders', body, 'POST');
    const db = getDbData();
    if (!db.riders) db.riders = [];
    const riderId = `rdr_${(body.phone || '').slice(-10)}`;
    const newRider = {
      id: riderId,
      rider_id: riderId,
      full_name: body.fullName || body.name || 'Delivery Partner',
      phone: body.phone ? (body.phone.startsWith('+91') ? body.phone : `+91${body.phone.slice(-10)}`) : '+919991416180',
      vehicle_number: (body.vehicleNumber || 'HR-26-AB-6180').toUpperCase(),
      vehicle_type: body.vehicleType || 'TWO_WHEELER',
      status: 'ACTIVE',
      aadhaar_number: body.aadhaarNumber || 'XXXX-XXXX-1234',
      created_at: new Date().toISOString()
    };
    const existingIdx = db.riders.findIndex(r => r.phone === newRider.phone || r.id === riderId);
    if (existingIdx >= 0) {
      db.riders[existingIdx] = { ...db.riders[existingIdx], ...newRider };
    } else {
    ...
    try {
      ...
      if (clean10.length === 10) {
        ...
        const fName = (newRider.full_name || 'Delivery Partner').replace(/'/g, "''");
        const vNum = (newRider.vehicle_number || 'HR-26-AB-6180').replace(/'/g, "''");
        const vType = (newRider.vehicle_type || 'TWO_WHEELER').replace(/'/g, "''");
        const sql = `INSERT INTO riders (id, rider_id, phone, full_name, vehicle_number, vehicle_type, tier, status, created_at, updated_at) VALUES ('rdr_${clean10}', 'rdr_${clean10}', '${fullPhone}', '${fName}', '${vNum}', '${vType}', 'STANDARD', 'ACTIVE', NOW(), NOW()) ON CONFLICT (phone) DO UPDATE SET full_name = EXCLUDED.full_name, vehicle_number = EXCLUDED.vehicle_number, status = 'ACTIVE', updated_at = NOW();`;
        const dbUrl = process.env.DATABASE_URL || 'postgresql://postgres.yjahldakwazqquznngsz:Jigsi%40311103@aws-0-ap-northeast-2.pooler.supabase.com:6543/postgres';
        if (dbUrl) execSync(`psql "${dbUrl}" -c "${sql}" 2>/dev/null || true`);
        execSync(`psql -d commerceos_test -c "${sql}" 2>/dev/null || true`);
        execSync(`psql -d commerce_os -c "${sql}" 2>/dev/null || true`);
      }
    } catch (_) {}

    res.writeHead(201, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ ok: true, message: 'Rider enrolled successfully and activated for fleet dispatch.', rider: newRider }));
    return;
  }

  // PATCH /api/v1/seller/riders/:id/status
  const riderStatusMatch = pathname.match(/^\/api\/v1\/seller\/riders\/([^/]+)\/status$/);
  if (riderStatusMatch && (req.method === 'PATCH' || req.method === 'PUT')) {
    const rId = decodeURIComponent(riderStatusMatch[1]);
    const body = await parseJsonBody(req);
    await postToGateway(`/api/v1/seller/riders/${encodeURIComponent(rId)}/status`, body, 'PATCH');
    const db = getDbData();
    if (!db.riders) db.riders = [];
    const r = db.riders.find(x => x.id === rId || x.rider_id === rId);
    if (r) {
      r.status = body.status || (r.status === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE');
      saveDbData(db);
    }
    try {
      const { execSync } = require('child_process');
      const newStatus = (body.status || (r && r.status === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE')).replace(/'/g, "''");
      const cleanRId = rId.replace(/'/g, "''");
      const sql = `UPDATE riders SET status = '${newStatus}', updated_at = NOW() WHERE id = '${cleanRId}' OR rider_id = '${cleanRId}';`;
      const dbUrl = process.env.DATABASE_URL || 'postgresql://postgres.yjahldakwazqquznngsz:Jigsi%40311103@aws-0-ap-northeast-2.pooler.supabase.com:6543/postgres';
      if (dbUrl) execSync(`psql "${dbUrl}" -c "${sql}" 2>/dev/null || true`);
      execSync(`psql -d commerceos_test -c "${sql}" 2>/dev/null || true`);
      execSync(`psql -d commerce_os -c "${sql}" 2>/dev/null || true`);
    ...
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ ok: true, rider: r || { id: rId, status: body.status } }));
    return;
  }
