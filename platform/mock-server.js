...
async function handleRequest(port, req, res) {
  ...
  (async () => {
    ...
    if (port === 8080) {
      ...
      if (path.includes('/interactions') || path.includes('/safety') || path.includes('/ai')) {
        return json(res, 200, {
      ...
      const fallbackRoute = GATEWAY_ROUTES.find((r) => path.startsWith(r.prefix));
      if (fallbackRoute && fallbackRoute.port !== 8080) {
        return handleRequest(fallbackRoute.port, req, res);
      }
    }
