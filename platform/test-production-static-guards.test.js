...
// Guard 79: rider_telemetry schema strictly enforces delivery_id NOT NULL and sequence_number BIGINT NOT NULL
checkGuard('rider_telemetry schema strictly enforces delivery_id NOT NULL and sequence_number BIGINT NOT NULL', () => {
  assert.ok(schemaContent.includes('delivery_id VARCHAR(64) NOT NULL'), 'rider_telemetry.delivery_id must be NOT NULL');
  assert.ok(schemaContent.includes('sequence_number BIGINT NOT NULL'), 'rider_telemetry.sequence_number must be BIGINT NOT NULL');
});

// Guard 80: rider_telemetry schema strictly enforces unique constraint on (delivery_id, sequence_number)
checkGuard('rider_telemetry schema strictly enforces unique constraint on (delivery_id, sequence_number)', () => {
  assert.ok(schemaContent.includes('uq_telemetry_delivery_seq UNIQUE(delivery_id, sequence_number)'), 'rider_telemetry must define uq_telemetry_delivery_seq');
});

// Guard 81: delivery_sessions schema strictly includes waypoints JSONB NOT NULL DEFAULT '[]'::jsonb
checkGuard('delivery_sessions schema strictly includes waypoints JSONB NOT NULL', () => {
  assert.ok(schemaContent.includes("waypoints JSONB NOT NULL DEFAULT '[]'::jsonb"), 'delivery_sessions.waypoints must be JSONB NOT NULL');
  assert.ok(schemaContent.includes('route_version INT NOT NULL DEFAULT 1'), 'delivery_sessions.route_version must be INT NOT NULL');
});

// Guard 82: TransactionalTelemetryRepository strictly defines getLatestTelemetryForDelivery
checkGuard('TransactionalTelemetryRepository strictly defines getLatestTelemetryForDelivery', () => {
  const telemClassMatch = repoContent.match(/class TransactionalTelemetryRepository[\s\S]*?class LocalDevelopmentTelemetryRepository/);
  assert.ok(telemClassMatch, 'TransactionalTelemetryRepository must exist');
  const telemCode = telemClassMatch[0];
  assert.ok(telemCode.includes('getLatestTelemetryForDelivery(deliveryId)'), 'Must define getLatestTelemetryForDelivery');
  assert.ok(telemCode.includes('WHERE delivery_id = $1'), 'Must filter telemetry by delivery_id');
});

// Guard 83: TransactionalTelemetryRepository.recordTelemetry strictly supports canonical object contract
checkGuard('TransactionalTelemetryRepository.recordTelemetry strictly supports canonical object contract', () => {
  const telemClassMatch = repoContent.match(/class TransactionalTelemetryRepository[\s\S]*?class LocalDevelopmentTelemetryRepository/);
  assert.ok(telemClassMatch, 'TransactionalTelemetryRepository must exist');
  const telemCode = telemClassMatch[0];
  assert.ok(telemCode.includes('data.deliveryId || data.delivery_id'), 'recordTelemetry must extract deliveryId from object');
  assert.ok(telemCode.includes('data.sequenceNumber || data.sequence_number'), 'recordTelemetry must extract sequenceNumber from object');
  assert.ok(telemCode.includes('ON CONFLICT (delivery_id, sequence_number) DO NOTHING'), 'recordTelemetry must enforce immutable telemetry packets via ON CONFLICT DO NOTHING');
});

// Guard 84: location-tracking.js contains ZERO synthetic speed (20) or distance assumptions (0.8, 2.0)
checkGuard('location-tracking.js contains ZERO synthetic speed (20) or distance assumptions (0.8, 2.0)', () => {
  const locTrackingPath = path.join(__dirname, '../platform/location-tracking.js');
  const locTrackingContent = fs.readFileSync(locTrackingPath, 'utf8');
  assert.ok(!locTrackingContent.includes('|| 20'), 'location-tracking.js must not invent 20 km/h speed');
  assert.ok(!locTrackingContent.includes('|| 0.8'), 'location-tracking.js must not invent 0.8 km distance');
  assert.ok(!locTrackingContent.includes('|| 2.0'), 'location-tracking.js must not invent 2.0 km distance');
});

// Guard 85: Production server active and session tracking endpoints strictly query delivery-scoped telemetry
checkGuard('Production server tracking endpoints strictly query delivery-scoped telemetry without rider fallback', () => {
  const activeDeliveryBlock = prodServerContent.slice(
    prodServerContent.indexOf('Customer Active Delivery Tracking: GET /api/v1/orders/active-delivery'),
    prodServerContent.indexOf('POST /api/v1/delivery/(session/)?:id/verify-otp')
  );
  assert.ok(activeDeliveryBlock.length > 0, 'Active delivery block must exist');
  assert.ok(activeDeliveryBlock.includes('getLatestTelemetryForDelivery(deliveryId)'), 'Active delivery tracking must query by deliveryId');
  assert.strictEqual(activeDeliveryBlock.includes('getLatestTelemetry(delivery.rider_id'), false, 'Active delivery tracking must NOT fall back to rider-wide telemetry');
});

// Guard 86: Production telemetry endpoint strictly mandates sequenceNumber and rejects missing sequence
checkGuard('Production telemetry endpoint strictly mandates positive sequenceNumber', () => {
  assert.ok(prodServerContent.includes('INVALID_TELEMETRY_SEQUENCE'), 'Telemetry ingestion must reject missing sequenceNumber with INVALID_TELEMETRY_SEQUENCE');
  assert.ok(!prodServerContent.includes('body.sequenceNumber || Date.now()'), 'Telemetry ingestion must not invent synthetic sequenceNumber from Date.now()');
});

// Guard 87: ProductionSseBroadcaster guards pg_notify payload size ceiling
checkGuard('ProductionSseBroadcaster guards pg_notify 8000-byte ceiling', () => {
  assert.ok(prodServerContent.includes('Buffer.byteLength(serialized, \'utf8\') > 7000'), 'ProductionSseBroadcaster must check payload byte length before pg_notify');
  assert.ok(prodServerContent.includes('isOversized: true'), 'ProductionSseBroadcaster must flag oversized payloads for remote database retrieval');
});

// Guard 88: ProductionSseBroadcaster isolates hot telemetry from heavy permanent storage
checkGuard('ProductionSseBroadcaster isolates high-frequency hot telemetry from heavy DB storage', () => {
  assert.ok(prodServerContent.includes("const isHotTelemetry = (event === 'TRACKING_UPDATE' || event === 'LOCATION_UPDATE');"), 'Broadcaster must identify hot telemetry events');
  assert.ok(prodServerContent.includes('this.hotLocationBus.publish('), 'Broadcaster must route hot telemetry to HotLocationBus');
});

// Guard 89: Schema contract strictly requires sequence_number on rider_telemetry table
checkGuard('Schema contract strictly requires sequence_number on rider_telemetry table', () => {
  const schemaContractPath = path.join(__dirname, 'test-postgres-live/schema-contract.test.js');
  const schemaContractContent = fs.readFileSync(schemaContractPath, 'utf8');
  assert.ok(schemaContractContent.includes("'sequence_number'"), 'schema-contract.test.js must require sequence_number column');
});

// Guard 90: ProductionSseBroadcaster skips pg_notify on hot high-frequency GPS telemetry
checkGuard('ProductionSseBroadcaster skips pg_notify on hot high-frequency GPS telemetry', () => {
  const broadcastCode = prodServerContent.slice(
    prodServerContent.indexOf('async broadcast('),
    prodServerContent.indexOf('const fcmSenderInstance')
  );
  assert.ok(broadcastCode.includes('if (isHotTelemetry)'), 'Broadcaster must check isHotTelemetry condition');
  assert.ok(broadcastCode.indexOf('this.hotLocationBus.publish') < broadcastCode.indexOf('pg_notify'), 'Hot telemetry must be routed to hot bus before durable pg_notify branch');
});

// Guard 91: location-tracking.js provides etaMode, predictive kinematics and staleDurationSeconds in DTO
checkGuard('location-tracking.js provides etaMode and predictive kinematics in tracking DTO', () => {
  const locTrackingPath = path.join(__dirname, '../platform/location-tracking.js');
  const locTrackingContent = fs.readFileSync(locTrackingPath, 'utf8');
  assert.ok(locTrackingContent.includes('etaMode,'), 'DTO must include etaMode');
  assert.ok(locTrackingContent.includes('isPredictiveMotionEnabled:'), 'DTO must include isPredictiveMotionEnabled');
  assert.ok(locTrackingContent.includes('staleDurationSeconds:'), 'DTO must include staleDurationSeconds');
});

// Guard 92: ProductionSseBroadcaster instantiates dedicated HotLocationBus
checkGuard('ProductionSseBroadcaster defines and binds dedicated HotLocationBus', () => {
  assert.ok(prodServerContent.includes('class HotLocationBus'), 'Must define HotLocationBus for hot location transport');
  assert.ok(prodServerContent.includes('this.hotLocationBus = new HotLocationBus'), 'ProductionSseBroadcaster must bind HotLocationBus instance');
});

// Guard 93: TransactionalTelemetryRepository strictly throws on invalid payload and reports duplicate status
checkGuard('TransactionalTelemetryRepository strictly throws on invalid payload and reports duplicate status', () => {
  assert.ok(repoContent.includes('INVALID_TELEMETRY_PAYLOAD'), 'recordTelemetry must throw INVALID_TELEMETRY_PAYLOAD on invalid input');
  assert.ok(repoContent.includes('INVALID_TELEMETRY_IDENTITY'), 'recordTelemetry must throw INVALID_TELEMETRY_IDENTITY on missing riderId/deliveryId');
  assert.ok(repoContent.includes('duplicate: !isInserted'), 'recordTelemetry must return duplicate status');
});

// Guard 94: ZomatoDarkMapView.kt implements predictive vehicle dead-reckoning motion engine
checkGuard('ZomatoDarkMapView.kt implements predictive vehicle dead-reckoning motion engine', () => {
  const mapViewPath = path.join(__dirname, '../apps/android/app/src/main/java/com/commerceos/android/ui/orders/ZomatoDarkMapView.kt');
  const mapViewContent = fs.readFileSync(mapViewPath, 'utf8');
  assert.ok(mapViewContent.includes('function applyPredictiveVehicleMotion'), 'ZomatoDarkMapView must implement applyPredictiveVehicleMotion');
  assert.ok(mapViewContent.includes('maxDeadReckoningWindowMs'), 'Must configure dead reckoning prediction window');
});

// Guard 95: production-server.js implements periodic dynamic routing ETA refresh against OSRM
checkGuard('production-server.js implements periodic dynamic routing ETA refresh against OSRM', () => {
  assert.ok(prodServerContent.includes('(nowMs - lastEtaRefresh > 15000) || (distSinceLastRefreshMeters >= 150)'), 'Must implement throttled dynamic ETA refresh loop');
});

// Guard 96: RiderAuthScreen.kt never pretends OTP sent on failure and has zero mock/default OTP literals
checkGuard('RiderAuthScreen.kt never pretends OTP sent on failure and has zero mock/default OTP literals', () => {
  const riderAuthPath = path.join(__dirname, '../apps/android/rider-app/src/main/java/com/commerceos/rider/ui/RiderAuthScreen.kt');
  const riderAuthContent = fs.readFileSync(riderAuthPath, 'utf8');
  assert.ok(!riderAuthContent.includes('mock_ch'), 'Must NOT create synthetic mock challenge');
  assert.ok(!riderAuthContent.includes('Default: 123456'), 'Must NOT expose default 123456 OTP hint');
  assert.ok(!riderAuthContent.includes('OTP sent: 123456'), 'Must NOT show fake OTP sent toast with 123456');
});

// Guard 97: OfferPayloadValidator.kt strictly rejects missing riderId with zero hardcoded defaults
checkGuard('OfferPayloadValidator.kt strictly rejects missing riderId with zero hardcoded defaults', () => {
  const validatorPath = path.join(__dirname, '../apps/android/rider-app/src/main/java/com/commerceos/rider/model/OfferPayloadValidator.kt');
  const validatorContent = fs.readFileSync(validatorPath, 'utf8');
  assert.ok(!validatorContent.includes('?: "rdr_rewari_01"'), 'Offer validator must NOT fall back to rdr_rewari_01');
});

// Guard 98: CartViewModel.kt enforces server-authoritative cart mutations with rollback on failure
checkGuard('CartViewModel.kt enforces server-authoritative cart mutations with rollback on failure', () => {
  const cartVmPath = path.join(__dirname, '../apps/android/app/src/main/java/com/commerceos/android/viewmodel/CartViewModel.kt');
  const cartVmContent = fs.readFileSync(cartVmPath, 'utf8');
  assert.ok(!cartVmContent.includes('is ApiResult.Failure -> {\n                        cartItems = cartItems + cartItem'), 'CartViewModel must NOT add item to cartItems on ApiResult.Failure');
  assert.ok(cartVmContent.includes('CartAddEvent.Failure(errMsg)'), 'CartViewModel must emit CartAddEvent.Failure on ApiResult.Failure');
});

// Guard 99: RiderDeliveryRepository and ForegroundService contain zero loginAsRider auto-login bypasses
checkGuard('Rider apps contain zero loginAsRider auto-login bypasses', () => {
  const repoPath = path.join(__dirname, '../apps/android/rider-app/src/main/java/com/commerceos/rider/repository/RiderDeliveryRepository.kt');
  const repoContent = fs.readFileSync(repoPath, 'utf8');
  const servicePath = path.join(__dirname, '../apps/android/rider-app/src/main/java/com/commerceos/rider/service/RiderForegroundLocationService.kt');
  const serviceContent = fs.readFileSync(servicePath, 'utf8');
  assert.ok(!repoContent.includes('fun loginAsRider'), 'RiderDeliveryRepository must NOT contain loginAsRider bypass');
  assert.ok(!serviceContent.includes('loginAsRider'), 'RiderForegroundLocationService must NOT auto-login as rider');
});

// Guard 100: RiderDeliveryRepository contains zero fallback transitionState calls on arrive/pickup/customer arrival failures
checkGuard('RiderDeliveryRepository enforces fail-closed domain actions with zero fallback state transitions', () => {
  const repoPath = path.join(__dirname, '../apps/android/rider-app/src/main/java/com/commerceos/rider/repository/RiderDeliveryRepository.kt');
  const repoContent = fs.readFileSync(repoPath, 'utf8');
  assert.ok(!repoContent.includes('transitionState('), 'RiderDeliveryRepository must NOT call fallback transitionState');
  assert.ok(!repoContent.includes('/api/v1/delivery/$deliveryId/transition'), 'Must NOT call nonexistent delivery transition endpoint');
});

// Guard 101: production-server.js rider profile endpoint returns authentic database fields without synthetic defaults
checkGuard('production-server.js rider profile returns authentic database fields without synthetic defaults', () => {
  const prodServerPath = path.join(__dirname, 'server/production-server.js');
  const serverCode = fs.readFileSync(prodServerPath, 'utf8');
  const profileSectionMatch = serverCode.match(/\/api\/v1\/delivery\/rider\/profile[\s\S]*?shift-status/);
  assert.ok(profileSectionMatch, 'Rider profile route not found');
  const profileCode = profileSectionMatch[0];
  assert.strictEqual(profileCode.includes("'HR-26-AB-1234'"), false, 'Must NOT hardcode vehicle HR-26-AB-1234');
  assert.strictEqual(profileCode.includes('rating: 4.9'), false, 'Must NOT hardcode rating 4.9');
  assert.strictEqual(profileCode.includes('STORE_REWARI_01'), false, 'Must NOT hardcode STORE_REWARI_01');
});
