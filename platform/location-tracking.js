...
function buildEnrichedTrackingDTO(session, rawTelemetry, fallbackPresence = null, waypoints = []) {
  ...
  return {
    ...
    order_id: session.orderId || session.order_id,
    deliveryId: session.deliveryId || session.delivery_id,
    delivery_id: session.deliveryId || session.delivery_id,
    status: session.state || session.status || 'PLACED',
    state: session.state,
    stage,
    orderStatus: session.order_status || session.orderStatus || session.state || 'PLACED',
    deliveryOtp: session.delivery_otp || session.deliveryOtp || session.otp || null,
    delivery_otp: session.delivery_otp || session.deliveryOtp || session.otp || null,
    isLiveTelemetryAvailable: Boolean(isAssigned && telemetry),
    is_live_telemetry_available: Boolean(isAssigned && telemetry),
    riderName: isAssigned ? (session.riderName || session.rider_name) : null,
    rider_name: isAssigned ? (session.riderName || session.rider_name) : null,
    riderPhone: isAssigned ? (session.riderPhone || session.rider_phone) : null,
    rider_phone: isAssigned ? (session.riderPhone || session.rider_phone) : null,
    riderVehicle: isAssigned ? (session.riderVehicle || session.rider_vehicle) : null,
    rider_vehicle: isAssigned ? (session.riderVehicle || session.rider_vehicle) : null,
    merchantLat: mLat,
    merchant_lat: mLat,
    merchantLng: mLng,
    merchant_lng: mLng,
    ...
    rider_lat: riderCurrentLat,
    riderLng: riderCurrentLng,
    rider_lng: riderCurrentLng,
    riderHeading: telemetry ? Number(telemetry.heading || telemetry.bearing || 0) : 0,
    rider_heading: telemetry ? Number(telemetry.heading || telemetry.bearing || 0) : 0,
    riderBearing: telemetry ? Number(telemetry.heading || telemetry.bearing || 0) : 0,
    rider_bearing: telemetry ? Number(telemetry.heading || telemetry.bearing || 0) : 0,
    speedKmh: telemetry ? Number(telemetry.speedKmh || telemetry.speed || 0) : 0,
    speed_kmh: telemetry ? Number(telemetry.speedKmh || telemetry.speed || 0) : 0,
    etaMinutes: etaMins,
    eta_minutes: etaMins,
    telemetrySource,
