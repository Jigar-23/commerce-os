...
function runTest() {
  ...
  try {
    ...
    // Execute package installation in clean temp directory with offline cache support
    let installResult = spawnSync('pnpm', ['install', '--prod', '--ignore-scripts', '--prefer-offline'], {
      cwd: tempDir,
      encoding: 'utf8',
      timeout: 30000
    });
    ...
    if (installResult.status !== 0) {
      ...
      installResult = spawnSync('npm', ['install', '--omit=dev', '--ignore-scripts', '--prefer-offline'], {
        cwd: tempDir,
        encoding: 'utf8',
        timeout: 30000
      });
    ...
    if (installResult.status === 0) {
      ...
      // If runner environment has no internet access or isolated sandbox network, verify direct require resolution
      const jwtPath = require.resolve('jsonwebtoken');
      const pgPath = require.resolve('pg');
      assert.ok(jwtPath, 'jsonwebtoken must resolve');
      assert.ok(pgPath, 'pg must resolve');
      const jwt = require('jsonwebtoken');
      const { Pool } = require('pg');
      assert.strictEqual(typeof jwt.verify, 'function');
      assert.strictEqual(typeof Pool, 'function');
      console.log('  ✅ PASS: Production dependencies verified in sandbox environment (offline resolver)');
    }
  } finally {
    try {
