package com.commerceos.customer.security;
...
/**
 * Enterprise-grade, Fail-Closed Cryptographic JWT Validator.
 * Enforces HMAC-SHA256 signature verification, algorithm allowlisting,
 * temporal boundaries (exp/nbf), and typed claim extraction via ObjectMapper.
 * Zero hardcoded secret fallbacks: fails fast if JWT_SECRET is not configured.
 */
@Component
public class JwtAuthValidator {
    ...
    public JwtAuthValidator() {
        ...
        if (secret == null || secret.isBlank()) {
            // Fail startup immediately in any production or deployed environment
    ...
    public Optional<AuthenticatedPrincipal> authenticate(String authHeader) {
        ...
        try {
            // 1. Parse and Validate Header via Jackson
            ...
            if (!"HS256".equalsIgnoreCase(alg)) {
                return Optional.empty(); // Strict algorithm allowlist (HS256 only)
            }
            ...
            if (!MessageDigest.isEqual(calculatedSig, providedSig)) {
                return Optional.empty(); // Cryptographic signature mismatch
            }

            // 3. Parse and Validate Payload via Jackson
            byte[] payloadBytes = Base64.getUrlDecoder().decode(payloadB64);
            JsonNode payloadNode = objectMapper.readTree(payloadBytes);

            long nowSeconds = Instant.now().getEpochSecond();

            // 4. Temporal Expiration (exp) Validation
            ...
            if (payloadNode.has("nbf")) {
                ...
                if (nbf > 0 && nbf > nowSeconds) {
                    return Optional.empty(); // Token not yet active
                }
            }
            ...
            if (subject.isBlank()) {
                return Optional.empty(); // Missing subject
            }

            // 7. Typed Roles Extraction
