...
public class CustomerController {
    ...
    @PostMapping("/{customerId}/addresses/{addressId}/default-shipping")
    public ResponseEntity<CustomerAddress> setDefaultShippingAddressPost(
            @RequestHeader(value = "Authorization", required = false) String authHeader,
            @PathVariable UUID customerId,
            @PathVariable UUID addressId
    ) {
        return setDefaultShippingAddress(authHeader, customerId, addressId);
    }
    ...
    public ResponseEntity<?> updateAddress(
        ...
        return addressRepository.findById(addressId).map(existing -> {
            if (existing.getCustomerProfile() != null && !existing.getCustomerProfile().getId().equals(customerId)) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN).body("FORBIDDEN: Address does not belong to customer.");
            }
            if (updated.getStreetAddress() != null) existing.setStreetAddress(updated.getStreetAddress());
            ...
            if (updated.getAddressType() != null) existing.setAddressType(updated.getAddressType());
            if (Boolean.TRUE.equals(updated.getIsDefaultShipping()) || Boolean.TRUE.equals(updated.getIsDefault())) {
                List<CustomerAddress> all = addressRepository.findByCustomerProfileId(customerId);
                for (CustomerAddress a : all) {
                    a.setIsDefaultShipping(a.getId().equals(addressId));
    ...
    public ResponseEntity<?> deleteAddress(
        ...
        return addressRepository.findById(addressId).map(address -> {
            if (address.getCustomerProfile() != null && !address.getCustomerProfile().getId().equals(customerId)) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN).body("FORBIDDEN: Address does not belong to customer.");
            }
            addressRepository.delete(address);
            return ResponseEntity.ok(java.util.Map.of("deleted", true, "addressId", addressId.toString(), "ok", true, "deletedId", addressId.toString()));
        }).orElse(ResponseEntity.ok(java.util.Map.of("deleted", true, "addressId", addressId.toString(), "ok", true, "deletedId", addressId.toString())));
    }
